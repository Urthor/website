...
pub fn recipe_landing(context: Scope) -> Element {
    context.render(rsx! {
        body {
            p { class: "text-[#8B4513] text-2xl text-center", "Recipe Landing Page Component." }
...
fn landing_grid(context: Scope) -> Element {
    ...
    context.render(rsx! {
        div { class: "flex justify-center mt-90",
            ...
            Link { to: "/my_first_recipe", class: "text-[#8B4515] text-2xl text-center", "My first recipe.  Start of Recipe Grid Component." }
        }
    })
}